<template>
  <div class="search-input" :class="{
    ...
    <input ref="inputRef" class="search-input__control" type="search" :value="modelValue || ''"
    ...
    <button v-if="allowClear && modelValue" type="button" class="search-input__clear" :disabled="isDisabled"
      @click="handleClear">
      ×
    </button>
...
const triggerSearch = () => {
  emit('search', props.modelValue?.toString().trim() ?? '');
};
