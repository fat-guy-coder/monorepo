  ...
  <div class="custom-select" ref="selectRef">
    <div class="select-input" :class="{ 'is-open': isOpen }" @click="toggleDropdown">
      <input
        ...
        @click.stop
      />
      <span v-else-if="selectedValue" class="selected-value">{{ selectedLabel }}</span>
    ...
    <transition name="slide-fade">
      <div v-if="isOpen" class="select-dropdown">
        <ul v-if="filteredOptions.length > 0">
          <li
            v-for="option in filteredOptions as Option[] | any"
        ...
        <div v-else class="no-data">无匹配选项</div>
      </div>
    </transition>
  </div>
</template>
...
const toggleDropdown = () => {
  ...
  if (isOpen.value && props.filterable) {
    setTimeout(() => filterInputRef.value?.focus(), 0);
  }
};
