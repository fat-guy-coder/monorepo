<template>
  <div class="transfer-wrapper" :class="{ 'is-disabled': disabled }">
    <!-- Source Panel -->
    <TransferPanel :title="titles[0] || '源列表'" :options="sourceOptions" :checked-keys="sourceChecked"
      @update:checked-keys="sourceChecked = $event" />

    <!-- Controls -->
    <div class="transfer-controls">
      <Button type="primary" class="control-btn" :disabled="sourceChecked.length === 0" @click="moveToTarget">
        &gt;
      </Button>
      <Button type="primary" class="control-btn" :disabled="targetChecked.length === 0" @click="moveToSource">
        &lt;
      </Button>
    </div>

    <!-- Target Panel -->
    <TransferPanel :title="titles[1] || '目标列表'" :options="targetOptions" :checked-keys="targetChecked"
