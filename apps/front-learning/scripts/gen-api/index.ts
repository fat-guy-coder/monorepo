...
async function emitModuleApi(
  ...
  const refToTypeName = (ref: string) => resolveRefName(ref);
  ...
  for (const [name, schema] of Object.entries(schemas)) {
    ...
    if (asInterface && ts.startsWith('{')) {
      typeChunks.push(`export interface ${name} ${ts}`);
    } else {
      typeChunks.push(`export type ${name} = ${ts};`);
    }
  }
  ...
  for (const [p, methods] of Object.entries(pathsObj)) {
    for (const [m, op] of Object.entries(methods ?? {})) {
      ...
      const respType = respSchema ? schemaToTs(respSchema, { refToTypeName }) : 'unknown';
