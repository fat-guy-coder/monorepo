...
export function smartSort<T>(
  ...
  for (let i = 1; i < sample.length; i++) {
    if ((compareFn ? compareFn(sample[i - 1], sample[i]) : (sample[i - 1] > sample[i] ? 1 : sample[i - 1] < sample[i] ? -1 : 0)) > 0)
      asc = false;
    if ((compareFn ? compareFn(sample[i - 1], sample[i]) : (sample[i - 1] > sample[i] ? 1 : sample[i - 1] < sample[i] ? -1 : 0)) < 0)
      desc = false;
  }
...
function merge<T>(left: T[], right: T[], compareFn?: (a: T, b: T) => number): T[] {
  ...
  while (i < left.length && j < right.length) {
    if (
      (compareFn ? compareFn(left[i], right[j]) : (left[i] > right[j] ? 1 : left[i] < right[j] ? -1 : 0)) <= 0
    ) {
      result.push(left[i++]);
    } else {
      result.push(right[j++]);
