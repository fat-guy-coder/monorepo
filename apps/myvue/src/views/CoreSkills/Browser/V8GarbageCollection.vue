        ...
        <div class="memory-visualization">
          <div class="memory-area" v-for="(area, index) in memoryAreas" :key="index">
            ...
            <div class="memory-blocks">
              <div v-for="(block, blockIndex) in area.blocks" :key="blockIndex" class="memory-block" :class="{
                'active': block.active,
        ...
        <div class="section-content">
          <div class="gc-intro">
            <div class="intro-card">
              ...
              <div>
                <h3>为什么需要垃圾回收？</h3>
                <p>JavaScript是一种高级语言，开发者不需要手动管理内存。V8引擎的垃圾回收器(GC)自动分配和释放内存，防止内存泄漏，确保应用性能。</p>
              </div>
            </div>
          ...
          <div class="gc-principle">
            ...
            <p>V8使用<strong>分代垃圾回收</strong>策略，将内存分为新生代(new space)和老生代(old space)。不同代采用不同的回收算法：</p>
      ...
      <section class="section">
        ...
        <div class="section-content">
          <div class="space-description">
            <p>新生代存放<strong>生命周期短</strong>的对象。大多数对象在这里被创建并很快被回收。V8使用<strong>Scavenge算法</strong>进行新生代垃圾回收。</p>
          </div>
  ...
  global.gc();
          ...
          <div class="memory-graph">
            ...
            <div class="graph-container">
              <div class="graph">
                <div v-for="(point, index) in memoryUsage" :key="index" class="data-point" :style="{
                  left: (index * 20) + 'px',
                  bottom: (point.usage * 0.8) + 'px'
                }"></div>
                <div class="gc-event" v-for="gc in gcEvents" :key="gc.id" :style="{ left: gc.position + 'px' }">
      ...
      <section class="section">
        ...
        <div class="section-content">
          ...
          <div class="practice-tip">
            ...
            <p><strong>关键提示：</strong> 不要尝试过度优化GC，V8的垃圾回收器已经高度优化。专注于编写清晰、无内存泄漏的代码，让GC完成它的工作。</p>
          </div>
        </div>
      </section>
    </div>
...
const memoryAreas = ref([
  {
    ...
    blocks: Array(30).fill(null).map((_, i) => ({
  ...
  },
  {
    ...
    color: "#1a73e8",
    icon: "fas fa-tree",
    blocks: Array(40).fill(null).map((_, i) => ({
  ...
  },
  {
    ...
    blocks: Array(5).fill(null).map((_, i) => ({
