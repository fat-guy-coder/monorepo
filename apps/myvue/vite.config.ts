...
/**
 * 确保 Node.js 环境中 localStorage 可用
 * 在 Node.js 环境中，localStorage 不是全局可用的，此函数创建一个基于内存的 localStorage 实现
 * 使用 Map 数据结构在内存中存储键值对，模拟浏览器 localStorage API
 */
const ensureNodeLocalStorage = () => {
  // 检查 globalThis 上是否已存在 localStorage
  ...
  // 创建符合 localStorage API 规范的内存存储对象
  const memoryStorage = {
    getItem: (key: string) => (store.has(key) ? store.get(key)! : null), // 获取值
    setItem: (key: string, value: string) => {
      store.set(key, String(value)) // 设置值
    },
    removeItem: (key: string) => {
      store.delete(key) // 删除指定键
    },
    clear: () => {
      store.clear() // 清空所有数据
    },
    key: (index: number) => Array.from(store.keys())[index] ?? null, // 根据索引获取键名
    get length() {
      return store.size // 返回存储项的数量
    },
  }

  // 将内存存储对象设置到 globalThis 上，使其全局可用
