  ...
  <div class="main-container">
    ...
    <div class="method-list">
      <div v-for="method in methods" :key="method.name" class="method-card">
        ...
        <div class="detail-item"><span class="label">优点:</span> {{ method.pros }}</div>
        <div class="detail-item"><span class="label">缺点:</span> {{ method.cons }}</div>
        <div class="recommendation">💡 推荐场景: {{ method.scenario }}</div>
      </div>
    </div>
  </div>
</template>
...
const methods: MethodInfo[] = [
  ...
  {
    name: '对象数组去重(根据某个属性值重复去重)',
    code: `const unique = (arr, key) => {
const map = new Map();
//或者const set = new Set();
const newArr = arr.filter((item,index) => {
  const value = item[key];
  if (map.has(value)) {//或者set.has(value)
     return false;
  }
  map.set(value, true);//或者set.add(value)
  return false;
  });
map.clear();//或者set.clear()
return newArr;
}; `,
    timeComplexity: 'O(n)',
