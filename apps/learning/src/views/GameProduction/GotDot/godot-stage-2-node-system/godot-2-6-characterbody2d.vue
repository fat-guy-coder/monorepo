<template>
  <div class="go-doc min-h-screen bg-linear-to-br from-slate-50 to-blue-50">
    ...
    <section id="sec-7" class="bg-white rounded-2xl shadow-md p-6 border border-slate-100 border-l-4 border-l-blue-400"><h2 class="text-lg font-semibold text-slate-800 mb-4 flex items-center gap-2"><span class="w-8 h-8 bg-blue-100 text-blue-700 rounded-lg flex items-center justify-center text-sm">🕹️</span>实战参考：blitz 的 player.gd</h2><p class="text-slate-600 mb-3 leading-relaxed">本文档的所有概念（重力、跳跃缓冲、coyote time、可变跳跃高度、空中加速度）都在 <code class="bg-slate-100 text-blue-700 px-1.5 py-0.5 rounded text-xs font-mono">apps/game/blitz/scripts/player.gd</code> 中有完整实现。打开对照看：</p><ul class="space-y-2 text-slate-600 mb-4"><li class="flex items-start gap-2"><span class="text-blue-500 mt-1">▸</span><span><strong>Phase 1 (本页)</strong> — 重力 + move_and_slide() → 看 phase1-starter</span></li><li class="flex items-start gap-2"><span class="text-blue-500 mt-1">▸</span><span><strong>Phase 2</strong> — 加上跳跃 + coyote time → 看 blitz player.gd:_process_normal()</span></li><li class="flex items-start gap-2"><span class="text-blue-500 mt-1">▸</span><span><strong>Phase 3</strong> — 冲刺无敌帧 + 可变跳跃高度 + 状态机 → 看 blitz player.gd 完整实现</span></li></ul><aside class="bg-blue-50 border-l-4 border-blue-400 rounded-r-xl p-4"><p class="text-sm text-blue-800"><strong>💡 学习建议：</strong>不要在浏览器里读完整个文件。打开 Godot → 开一个变量（如 gravity）→ 看文档理解含义 → 改值 → F5 看效果 → 继续下一个变量。</p></aside></section></main>
