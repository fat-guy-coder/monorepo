...
1. **按钮覆盖全部操作** — 文档讲了哪些算法/操作，动画就必须有对应按钮，一个不漏（链表讲反转/插入/删除/环检测，动画就要全有；堆讲插入/删除/建堆/堆排序，动画也要全有；排序讲快排/归并/堆排，动画也要全覆盖）
2. **附加信息附带到画布** — 长度/复杂度/指针状态/容量/数字变化/等式变化/中间状态，既要显示在状态 pill，也要标注在画布内的相关元素旁（如堆排序时在节点旁标注 `siftDown 3→7`、建堆时标注累计比较次数、快排时标注 `pivot=5 · swap 3↔7`）
